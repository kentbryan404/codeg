...
#
# 本地构建加速（都不改分发流程的语义）：
#   * 链接器优先 mold、其次 lld（release 档开了 LTO，链接是大头）。任一缺装即
#     跳过；设 CODEG_NO_LLD=1 可整体关闭。
#   * 装了 sccache 就自动挂上做跨次编译缓存（RUSTC_WRAPPER）；设
#     CODEG_NO_SCCACHE=1 关闭。
#   * `--fast` 关掉 LTO、把 codegen-units 放回 16，显著缩短编译，代价是产物
#     运行时性能下降——只用于本地迭代，不要拿它出分发包。
#
# 两个本地构建的坑，脚本已代处理：
...
if $fast; then
  ...
  # 用环境变量覆盖 cargo 的 release 档位，不动 Cargo.toml 里的分发配置。
  export CARGO_PROFILE_RELEASE_LTO=false
  export CARGO_PROFILE_RELEASE_CODEGEN_UNITS=16
  echo "[build] --fast：关闭 LTO、codegen-units=16（仅本次构建）"
fi
...
echo
echo "[build] 完成，产物位于 src-tauri/target/release/bundle："
